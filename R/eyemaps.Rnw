...
for(i in 1:length(prdatafiles)){
	...
	expstr <- ifelse(grepl("33", plot_labels[i]), "P150 RCS Rat", "P90 RCS Rat") #determine which experiment to conditionally determine part of the caption
	...
	rpe.plot.pr.rescue(infname,title=plot_labels[i], totalslides=eyesections,xlab="X axis", ylab="Y axis" )
	dev.off()
	cat("\\begin{center}\n")
	cat("\\begin{figure}\n")
	cat("\\includegraphics{", tmpfname, "}\n", sep="")
